...
{
   real sum = 0;       //
   sum += n - m > i ? Mat[0][i] * x_k[i + m] : 0;  
   sum += n - 1 > i ? Mat[1][i] * x_k[i + 1] : 0;  
   sum +=            Mat[2][i] * x_k[i];                      
   sum += 1 <= i ? Mat[3][i - 1] * x_k[i - 1] : 0; 
   sum += m <= i ? Mat[4][i - m] * x_k[i - m] : 0; 

   return x_k[i] + w * (b[i] - sum) / Mat[2][i];
}
...
real Matrix::discrepancy()
{
   ...
   for (int i = 0; i < n; i++)
   {
      sum2 += n - m > i ? Mat[0][i] * x[i + m] : 0;
      sum2 += n - 1 > i ? Mat[1][i] * x[i + 1] : 0;
      sum2 += Mat[2][i] * x[i];
      sum2 += 1 <= i ? Mat[3][i - 1] * x[i - 1] : 0;
      sum2 += m <= i ? Mat[4][i - m] * x[i - m] : 0;
